...
// A single-employee payroll run for exceptional payments such as a bonus,
// correction, or severance. It deliberately starts In Review: a payroll
// manager must still approve it before the payslip becomes visible or emailed.
export async function createOffCyclePayout(req, res, next) {
  try {
    const { employee: employeeId, grossAmount, category, reason } = req.body;
    const gross = Number(grossAmount);
    if (!employeeId || !Number.isFinite(gross) || gross <= 0 || !category || !reason?.trim()) {
      return res.status(400).json({ message: "Employee, a positive payout amount, category, and audit note are required." });
    }
    const employee = await Employee.findOne({ _id: employeeId, status: "Active" });
    if (!employee) return res.status(404).json({ message: "Select an active employee for this payout." });

    const now = new Date();
    const tax = Math.round(gross * 0.1 * 100) / 100;
    const net = Math.round((gross - tax) * 100) / 100;
    const run = await PayrollRun.create({
      periodName: `Off-cycle · ${category} · ${employee.employeeCode} · ${now.toLocaleDateString("en-CA")}`,
      startDate: now,
      endDate: now,
      payDate: now,
      status: "In Review",
      createdBy: req.user.id,
      totals: { gross, deductions: tax, net },
    });
    const payslip = await Payslip.create({
      payrollRun: run._id,
      employee: employee._id,
      earnings: { basicSalary: 0, allowances: 0, overtimePay: 0, bonus: gross, gross },
      deductions: { tax, unpaidLeave: 0, total: tax },
      netPay: net,
      flags: [`Off-cycle payout: ${category}`, `Audit note: ${reason.trim()}`],
    });
    await logAudit(req.user.id, "Created off-cycle payout", "PayrollRun", run._id, {
    ...
    res.status(201).json({ data: { run, payslip } });
  } catch (error) { next(error); }
}
