...
export const api = {
  ...
  createOffCyclePayout: (payload: { employee: string; grossAmount: number; category: string; reason: string }, token: string) => request<{ data: { run: any; payslip: any } }>('/payroll-runs/off-cycle', { method: 'POST', body: JSON.stringify(payload) }, token),
  ...
  updateContract: (id: string, payload: Record<string, unknown>, token: string) => request<{ data: any }>(`/contracts/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }, token),
