...
export function QuickActionModal({
  ...
  const [payoutCategory, setPayoutCategory] = useState('Relocation / Moving Bonus');
  ...
  const canManagePayroll = ['admin', 'hr_payroll_manager', 'hr_payroll_user'].includes(userRole);
  const canSubmitLeaveForOthers = ['admin', 'hr_manager', 'hr_payroll_manager', 'hr_payroll_user'].includes(userRole);

  React.useEffect(() => {
    if (!isOpen) return;
    api.employees(token).then((result) => setEmployees(result.data)).catch(() => setEmployees([]));
    if (!canManagePayroll) setActiveTab('leave_request');
  }, [isOpen, token, canManagePayroll]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeTab === 'new_employee') {
        ...
        if (!/^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@gmail\.com$/i.test(email.trim())) throw new Error('Use a valid Gmail address ending in @gmail.com.');
        ...
        if (canSubmitLeaveForOthers && !employee) throw new Error('Select an employee for this leave request.');
        const totalDays = Math.max(1, Math.round((new Date(endDate).getTime() - new Date(startDate).getTime()) / 86400000) + 1);
        await api.createLeaveRequest({ ...(canSubmitLeaveForOthers ? { employee: employee!._id } : {}), leaveType: leaveCategory, startDate, endDate, totalDays, reason: reason || 'Leave request submitted from dashboard.' }, token);
        onSuccessToast(canSubmitLeaveForOthers ? `Leave request for ${employee!.firstName} ${employee!.lastName} was submitted.` : 'Your leave request was submitted.');
      } else {
        if (!canManagePayroll) throw new Error('You do not have permission to create an off-cycle payout.');
        const employee = employees.find((item) => item._id === employeeName);
        if (!employee) throw new Error('Select an employee for this payout.');
        if (!Number.isFinite(Number(amount)) || Number(amount) <= 0) throw new Error('Enter a positive gross payout amount.');
        if (!reason.trim()) throw new Error('Provide a justification for the audit log.');
        await api.createOffCyclePayout({ employee: employee._id, grossAmount: Number(amount), category: payoutCategory, reason }, token);
        onSuccessToast(`Off-cycle payout for ${employee.firstName} ${employee.lastName} is ready for payroll approval.`);
      }
      await onRefresh();
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <div className={`grid ${canManagePayroll && canCreateEmployee ? 'grid-cols-3' : (canManagePayroll || canCreateEmployee) ? 'grid-cols-2' : 'grid-cols-1'} gap-2 p-1 bg-slate-100/90 rounded-xl border border-slate-200/80`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {activeTab === 'off_cycle' && (
            <div className="space-y-3.5">
              ...
              <span>This creates one employee's off-cycle run in <strong>In Review</strong>. The 10% tax estimate and draft payslip are created immediately; a payroll manager must approve it before payment and email delivery.</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={payoutCategory} onChange={(e) => setPayoutCategory(e.target.value)} className="w-full text-xs px-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
