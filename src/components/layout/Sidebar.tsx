...
export function Sidebar({
  ...
  useEffect(() => {
    const loadCounts = async () => {
      ...
      try {
        const leaves = await api.leaveRequests(token);
        setPendingLeaveCount(leaves.data.filter((leave) => leave.status === 'Pending').length);
  ...
  const sections = NAV_SECTIONS.map((section) => ({
    ...
    items: section.items
      ...
      .map((item) => item.id === 'employees' ? { ...item, badge: employeeCount } : item.id === 'time-off' ? { ...item, badge: pendingLeaveCount || undefined } : item.id === 'payroll' ? { ...item, badge: payrollCount === null ? undefined : `Cycle ${String(payrollCount).padStart(2, '0')}` } : item),
